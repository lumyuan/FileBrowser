...
public class AttachListPopupView extends AttachPopupView {
    ...
    public AttachListPopupView(@NonNull Context context, int bindLayoutId, int bindItemLayoutId, int imageColor, int textColor) {
        super(context);
        this.bindLayoutId = bindLayoutId;
        this.bindItemLayoutId = bindItemLayoutId;
        ...
        final EasyAdapter<String> adapter = new EasyAdapter<String>(Arrays.asList(data), bindItemLayoutId == 0 ? R.layout._xpopup_adapter_text : bindItemLayoutId) {
            ...
            protected void bind(@NonNull ViewHolder holder, @NonNull String s, int position) {
                ...
                if (iconIds != null && iconIds.length > position) {
                    if(imageView!=null){
                        if (imageColor != 0) imageView.setColorFilter(imageColor);
                ...
                if(bindItemLayoutId==0 ){
                    if(popupInfo.isDarkTheme){
                        holder.<TextView>getView(R.id.tv_text).setTextColor(textColor == 0 ? getResources().getColor(R.color._xpopup_white_color) : textColor);
                    }else {
                        holder.<TextView>getView(R.id.tv_text).setTextColor(textColor == 0 ? getResources().getColor(R.color._xpopup_dark_color) : textColor);
                    }
